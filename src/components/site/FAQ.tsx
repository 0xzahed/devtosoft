        ...
        <div className="mt-14">
          {faqs.map((f, i) => (
            <div key={f.q} className="border-t border-border last:border-b">
              ...
              >
                <span className="text-2xl text-foreground md:text-3xl">{f.q}</span>
                <span
                  className={`font-display text-3xl text-primary transition-transform ${
                    open === i ? "rotate-45" : ""
                  }`}
                >
                  +
              ...
              {open === i && (
                <p className="max-w-2xl pb-6 text-muted-foreground">{f.a}</p>
              )}
            </div>
          ))}
        </div>
